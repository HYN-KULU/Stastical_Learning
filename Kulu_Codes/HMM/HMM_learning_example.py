...
fig = plt.subplot(222)
plt.hist(rolls[gen_states == 0], label='fair', alpha=0.5,
         bins=np.arange(7) - 0.5, density=True)
plt.hist(rolls[gen_states == 1], label='loaded', alpha=0.5,
         bins=np.arange(7) - 0.5, density=True)
plt.title('Roll probabilities by state')
plt.xlabel('Count')
plt.ylabel('Roll')
plt.legend()
...
for idx in range(n_fits):
    ...
    # we need to initialize with random transition matrix probabilitset_ies
...
fig = plt.subplot(223)
plt.plot(gen_states[:500], label='generated')
plt.plot(states[:500] + 1.5, label='recovered')
plt.yticks([])

...
[m,n]=size(a);
reference_list=max(a')';  %求参考序列的取值
t=repmat(reference_list,[1,n])-a;  %求参考序列与每一个序列的差
mmin=min(min(t));   %计算最小差
mmax=max(max(t));  %计算最大差
rho=0.5; %分辨系数
coef=(mmin+rho*mmax)./(t+rho*mmax)  %计算灰色关联系数
relevance_deg=mean(coef)   %取等权重，计算关联度
[gsort,ind]=sort(relevance_deg,'descend')  %对关联度按照从大到小排序
